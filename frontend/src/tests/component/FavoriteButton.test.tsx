...
test('FavoriteButton shows login prompt when not logged in', () => {
    ...
    expect(customToast).toHaveBeenCalledWith('error', 'You need to login to add to favorites');
});
...
test('FavoriteButton handles error during favorite mutation', async () => {
    ...
    vi.mocked(useFavorite).mockReturnValueOnce({
        addFavoriteMutation: vi.fn().mockRejectedValue(new Error('Mutation error')),
